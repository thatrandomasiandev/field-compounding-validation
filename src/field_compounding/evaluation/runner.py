...
def resolve_config_path(module: int, configs_dir: str | Path = "configs") -> Path:
    configs_dir = Path(configs_dir)
    mod = str(module).zfill(2)
    exact = configs_dir / f"module_{mod}.yaml"
    if exact.is_file():
        return exact
    matches = sorted(configs_dir.glob(f"module_{mod}_*.yaml"))
    if matches:
        return matches[0]
    raise FileNotFoundError(f"no config for module {mod}")


def get_evaluation_seeds(config: dict[str, Any], n_trials: int | None = None) -> list[int]:
    seeds = config.get("evaluation", {}).get("seeds") or config.get("seeds") or [0]
    seeds = [int(s) for s in seeds]
    if n_trials and len(seeds) >= n_trials:
        return seeds[:n_trials]
    if n_trials:
        return list(range(n_trials))
    return seeds


def get_violation_values(config: dict[str, Any]) -> list[float]:
    if "violation_sweep" in config:
        return [float(v) for v in config["violation_sweep"]["values"]]
    if "violation_levels" in config:
        return [float(v) for v in config["violation_levels"]]
    return [0.0, 1.0]
...
def aggregate_trials(
    trial_results: list[dict[str, float]], metrics: list[str]
    ...
    for key in all_keys:
        values = [
            r[key] for r in trial_results if key in r and isinstance(r.get(key), (int, float))
        ]
        if not values:
            continue
...
def run_module_benchmark(
    ...
    results: dict[str, Any] = {
        ...
        "metadata": {
            ...
            "trace_path": dgp_params.get("trace_path"),
            "fast_mode": fast,
            "device": str(device),
                ...
                try:
                    if trial_timeout > 0:
                        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                            ...
                            trial_metrics = future.result(timeout=trial_timeout)
                    else:
